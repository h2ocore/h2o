...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    // * Disabled to avoid the EXPIRED stated on the old version after the activation of the SPORK 10
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if(coins.vout[vin.prevout.n].nValue != ActiveCollateral() * COIN) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d H2O, masternode=%s\n", ActiveCollateral(), vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when H2O Collateral tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for H2O Collateral tx -> 1 confirmation
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos)
{
...

    {
        LOCK(cs_main);
        BlockMap::iterator mi = mapBlockIndex.find(blockHash);
        if ((*mi).second && (*mi).second->nHeight < chainActive.Height() - (24*3)) {
